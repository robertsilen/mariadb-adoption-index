...
# 1. Read create_index_weights.csv
weights = pd.read_csv('index/create_index_weights.csv')

# New code to plot weights as a pie chart
...
for index, row in weights.iterrows():
    monthly_data = pd.read_csv(f'data/{row['path']}_monthly.csv')
...
# 5. Prepare the data for the table by removing all _index columns except for weighted_index
...
# Generate the list of months to include, ensuring the last month is correct
...
# New code to save filtered_data as a PNG file
...
plt.figure(figsize=(10, 5))
plt.plot(filtered_data.columns, filtered_data.loc['Weighted Index'], marker='o')  # Use columns for x-axis
