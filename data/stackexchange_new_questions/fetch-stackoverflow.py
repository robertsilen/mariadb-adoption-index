...
def get_mariadb_questions_by_month():
    ...
    # Optional API key to increase quota and reduce throttling
    api_key = os.getenv('STACKEXCHANGE_KEY') or os.getenv('STACKEXCHANGE_API_KEY')
    if api_key:
        params["key"] = api_key
    else:
        print("Warning: No Stack Exchange API key set. Set 'STACKEXCHANGE_KEY' or 'STACKEXCHANGE_API_KEY' to increase quota.")
...
def save_to_monthly_csv(questions):
    ...
    # Handle empty results or unexpected schema
    if df.empty or 'creation_date' not in df.columns:
        monthly_counts = pd.DataFrame(columns=['month', 'stackexchange_new_questions'])
        monthly_counts.to_csv('data/stackexchange_new_questions/monthly-stackoverflow.csv', index=False)
        print("No data or missing 'creation_date'. Wrote empty monthly CSV.")
        return
    ...
    # Sort monthly counts by oldest date first (ascending)
    monthly_counts = monthly_counts.sort_values(by='month', ascending=True)

    # Save results to CSV
    monthly_counts.to_csv('data/stackexchange_new_questions/monthly-stackoverflow.csv', index=False)
    print("Data has been saved to 'monthly-stackoverflow.csv'.")
